...
AND (SELECT next_time FROM auto_freeze WHERE id = 3) - '7 days'::interval - accepted - (SELECT CASE WHEN state = 'discussion' THEN verification_time + voting_time WHEN state = 'verification' THEN voting_time ELSE '00:00:00' END FROM policy WHERE id = policy_id) > '-00:05:00';
